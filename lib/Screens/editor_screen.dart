...
class _ItemControllers {
  ...
  _ItemControllers.fromItem(Map item)
      : description = TextEditingController(text: item['description']?.toString() ?? ''),
        ...
        unitPrice = TextEditingController(text: item['unit_price']?.toString() ?? '0'),
        discount = TextEditingController(text: item['discount']?.toString() ?? '0'),
        taxRate = TextEditingController(text: item['tax_rate']?.toString() ?? '0');

  Map<String, dynamic> toMap() {
    double parseDecimal(TextEditingController c) => double.tryParse(c.text.trim()) ?? 0;
    return {
...
class _EditorScreenState extends State<EditorScreen> {
  ...
  void initState() {
    super.initState();
    bundle = jsonDecode(jsonEncode(widget.initialBundle)) as Map<String, dynamic>;
    _initControllers();
  }
  ...
  void _initControllers() {
    ...
    final projectBrief = Map<String, dynamic>.from(bundle['project_brief'] ?? {});
    ...
    setController('brief_success_metrics', joinList(projectBrief['success_metrics']));
    ...
    setController('brief_acceptance', joinList(projectBrief['acceptance_criteria']));
    setController('brief_support_window', projectBrief['support_window']);

    _itemControllers = (bundle['items'] as List?)
            ?.map((e) => _ItemControllers.fromItem(Map<String, dynamic>.from(e)))
            .toList() ??
        [
    ...
    _milestoneControllers = (projectBrief['milestones'] as List?)
            ?.map((e) => _MilestoneControllers.from(Map<String, dynamic>.from(e)))
            .toList() ??
        [
    ...
    _billingControllers = (projectBrief['billing_plan'] as List?)
            ?.map((e) => _BillingPartControllers.from(Map<String, dynamic>.from(e)))
            .toList() ??
        [
          _BillingPartControllers.from({'when': 'On acceptance', 'percent': 50}),
  ...
  bool _applyForm({bool validate = true}) {
    ...
    if (items.isEmpty || items.any((i) => i['description'].toString().isEmpty)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Add at least one item with description.')),
      );
      return false;
    }
    ...
    for (final item in items) {
      ...
      if (qty <= 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Quantity must be positive for "${item['description']}".')),
        );
        return false;
      }
      if (discount < 0 || discount > qty * unitPrice) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Discount for "${item['description']}" exceeds line amount.')),
        );
        return false;
      }
    }
    ...
    final totals = Map<String, dynamic>.from(bundle['totals'] ?? {});
    totals['shipping'] = double.tryParse(_controllers['totals_shipping']!.text.trim()) ?? 0;
    totals['round_off'] = double.tryParse(_controllers['totals_round_off']!.text.trim()) ?? 0;
    totals['amount_in_words'] = _controllers['totals_amount_in_words']!.text.trim();
    ...
    final quotation = {
      ...
      'validity_days': int.tryParse(_controllers['quotation_validity_days']!.text.trim()),
      'offer_type': _controllers['quotation_offer_type']!.text.trim(),
      'pricing_options_enabled': _pricingOptionsEnabled,
      'advance_percent': int.tryParse(_controllers['quotation_advance_percent']!.text.trim()),
      ...
      'change_requests_rate': _controllers['quotation_change_rate']!.text.trim(),
    };

    final invoice = {
      'supply_date': _controllers['invoice_supply_date']!.text.trim(),
      'bill_to_ship_to_same': _billToShipSame,
      'ship_to_address': _billToShipSame ? '' : _controllers['invoice_ship_to']!.text.trim(),
    ...
    final projectBrief = {
      ...
      'timeline_days': int.tryParse(_controllers['brief_timeline_days']!.text.trim()) ?? 0,
    ...
    if (seller['name'].toString().isEmpty || seller['name'].toString().length < 2) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Seller business name is required (2-100 characters).')),
      );
      return false;
    }
    if (buyer['name'].toString().isEmpty || buyer['name'].toString().length < 2) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Buyer name is required (2-100 characters).')),
      );
      return false;
    }
    ...
    if (issue != null) {
      if (due != null && due.isBefore(issue)) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Due date must be on or after issue date.')),
        );
        return false;
      }
      if (validTill != null && validTill.isBefore(issue)) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Valid till date must be on or after issue date.')),
        );
        return false;
      }
    }
    ...
    if (_docType == 'TAX INVOICE' && docMeta['doc_no'].toString().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Invoice number is required for tax invoices.')),
      );
      return false;
    }

    if (_docType == 'PROJECT BRIEF') {
      if (projectBrief['title'].toString().isEmpty || projectBrief['objective'].toString().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Project brief requires a title and objective.')),
        );
        return false;
      }
      if ((projectBrief['scope'] as List).isEmpty || (projectBrief['deliverables'] as List).isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Add at least one scope item and deliverable.')),
        );
        return false;
      }
      ...
      if ((projectBrief['billing_plan'] as List).isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Add billing plan parts totaling 100%.')),
        );
        return false;
      }
      ...
      if (billingTotal != 100) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Billing plan percentages must total 100 (current: $billingTotal).')),
        );
        return false;
      }
      ...
      if ((projectBrief['timeline_days'] as num) <= 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Timeline days must be greater than zero.')),
        );
        return false;
      }
    }
  ...
  void _recalculateTotals() {
    ...
    totals['grand_total'] = double.parse((subtotal - discountTotal + taxTotal + shipping + roundOff).toStringAsFixed(2));
    bundle['totals'] = totals;
    setState(() {});
  }
  ...
  void _addItem() {
    setState(() {
      _itemControllers.add(_ItemControllers.fromItem({
        'description': 'New item',
        'qty': 1,
        'unit': 'pcs',
        'unit_price': 0,
  ...
  void _addMilestone() {
    setState(() {
      _milestoneControllers.add(
        _MilestoneControllers.from({'name': '', 'start': '', 'end': '', 'fee': 0}),
      );
    });
  }
  ...
  void _addBillingPart() {
    setState(() {
      _billingControllers.add(_BillingPartControllers.from({'when': '', 'percent': 0}));
    });
  }
  ...
  Future<void> _previewPdf() async {
    ...
    Navigator.of(context)
        .push(MaterialPageRoute(builder: (_) => PdfPreviewScreen(document: pdf, bundle: bundle)));
  }

  Future<void> _exportPdf() async {
    if (!_applyForm()) return;
    final pdf = await PDFGenerator.buildPdf(bundle);
    await Hive.box('history').add({'bundle': bundle, 'created': DateTime.now().toIso8601String(), 'type': 'export'});
    if (!mounted) return;
  ...
  Future<void> _share() async {
    if (!_applyForm()) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Share flow will be implemented in the mobile shell.')),
    );
  }
  ...
  void _generatePayLink() {
    ...
    if (upi.isEmpty || !upiRegex.hasMatch(upi)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Enter a valid UPI ID before generating the pay link.')),
      );
      return;
    }
  ...
  Future<void> _copyDeeplink() async {
    ...
    await Clipboard.setData(ClipboardData(text: _controllers['payment_deeplink']!.text.trim()));
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildTextField(String key, String label,
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        labelText: label,
        hintText: hint,
        suffixIcon: onTap != null ? const Icon(Icons.calendar_today_outlined) : null,
      ),
      validator: validator ??
          (value) {
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              ...
              Expanded(child: TextFormField(controller: ic.unit, decoration: const InputDecoration(labelText: 'Unit'))),
            ]),
            const SizedBox(height: 8),
            Row(children: [
              Expanded(
            ...
            const SizedBox(height: 8),
            TextFormField(controller: ic.hsn, decoration: const InputDecoration(labelText: 'HSN/SAC')),
            const SizedBox(height: 8),
            Row(
              children: [
                ...
                IconButton(
                  ...
                  onPressed: () {
                    setState(() {
                      _itemControllers.insert(index + 1, _ItemControllers.fromItem(_itemControllers[index].toMap()));
                    });
                  },
        ...
        child: Padding(
          ...
          child: Column(children: [
            TextFormField(
              ...
              validator: (value) => (value?.trim().isEmpty ?? true) ? 'Milestone name required' : null,
            ),
            const SizedBox(height: 8),
            Row(children: [
              Expanded(
        ...
        child: Padding(
          ...
          child: Row(children: [
            Expanded(
              ...
              child: TextFormField(
                controller: bc.when,
                decoration: const InputDecoration(labelText: 'Billing trigger *'),
                validator: (value) => (value?.trim().isEmpty ?? true) ? 'Provide the trigger' : null,
              ),
            ),
  ...
  PopupMenuButton<String> _buildOverflowMenu() {
    return PopupMenuButton<String>(
      ...
      itemBuilder: (context) => [
        const PopupMenuItem(value: 'duplicate_quote', child: Text('Duplicate as Quotation')),
        const PopupMenuItem(value: 'duplicate_invoice', child: Text('Duplicate as Tax Invoice')),
        const PopupMenuItem(value: 'duplicate_brief', child: Text('Duplicate as Project Brief')),
    ...
    return Scaffold(
      ...
      body: Form(
        ...
        child: SingleChildScrollView(
          padding: const EdgeInsets.all(12.0),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(_sectionTitle('Seller block'), style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            _buildTextField('seller_name', 'Business name', requiredField: true,
                validator: (value) {
              final trimmed = value?.trim() ?? '';
            ...
            _buildTextField('seller_address', 'Address', maxLines: 2, hint: '≤ 300 chars'),
            Row(children: [
              Expanded(child: _buildTextField('seller_email', 'Email')),
              const SizedBox(width: 8),
              Expanded(child: _buildTextField('seller_phone', 'Phone')),
            ]),
            Row(children: [
              Expanded(child: _buildTextField('seller_gstin', 'GSTIN', hint: '15-char GST number')),
              const SizedBox(width: 8),
              Expanded(child: _buildTextField('seller_pan', 'PAN', hint: '10-char PAN')),
            ]),
            ExpansionTile(
              ...
              children: [
                _buildTextField('seller_bank_account_name', 'Account name'),
                Row(children: [
                  Expanded(child: _buildTextField('seller_bank_account_no', 'Account number')),
                  const SizedBox(width: 8),
                  Expanded(child: _buildTextField('seller_bank_ifsc', 'IFSC code')),
                ]),
                _buildTextField('seller_bank_upi', 'UPI ID', hint: 'id@psp'),
              ],
            ),
            const SizedBox(height: 24),
            Text(_sectionTitle('Buyer block'), style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            _buildTextField('buyer_name', 'Client name', requiredField: true,
                validator: (value) {
              final trimmed = value?.trim() ?? '';
            ...
            Row(children: [
              Expanded(child: _buildTextField('buyer_email', 'Buyer email')),
              const SizedBox(width: 8),
              Expanded(child: _buildTextField('buyer_phone', 'Buyer phone')),
            ]),
            _buildTextField('buyer_address', 'Buyer address', maxLines: 2),
            _buildTextField('buyer_gstin', 'Buyer GSTIN'),
            const SizedBox(height: 24),
            Text(_sectionTitle('Document meta'), style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            _buildTextField('doc_no', 'Document number', requiredField: _docType == 'TAX INVOICE'),
            Row(children: [
              Expanded(child: _buildTextField('ref_no', 'Reference / Quote title')),
              const SizedBox(width: 8),
              Expanded(child: _buildTextField('po_no', 'PO number')),
            ]),
            ...
            Row(children: [
              Expanded(
                child: _buildTextField(
                  ...
                  requiredField: true,
                  readOnly: true,
                  onTap: () => _pickDate(_controllers['issue_date']!),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
              ...
              Expanded(
                child: _buildTextField(
                  ...
                  'Valid till',
                  readOnly: true,
                  onTap: () => _pickDate(_controllers['valid_till']!),
                ),
              ),
            ]),
            const SizedBox(height: 24),
            Text(_sectionTitle('Items'), style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            ..._buildItemRows(),
            Align(
            ...
            const SizedBox(height: 24),
            Text(_sectionTitle('Totals ($currency)'), style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            Wrap(
              spacing: 12,
              runSpacing: 12,
              children: [
                Chip(label: Text('Subtotal: ${totals['subtotal'] ?? 0}')),
                Chip(label: Text('Discount: ${totals['discount_total'] ?? 0}')),
                Chip(label: Text('Tax: ${totals['tax_total'] ?? 0}')),
                Chip(label: Text('Grand total: ${totals['grand_total'] ?? 0}')),
              ],
            ),
            const SizedBox(height: 8),
            _buildTextField('totals_shipping', 'Shipping', inputType: TextInputType.number),
            _buildTextField('totals_round_off', 'Round off', inputType: TextInputType.number,
                hint: _roundOffEnabled ? null : 'Round off disabled'),
            SwitchListTile(
              value: _roundOffEnabled,
              onChanged: (value) => setState(() => _roundOffEnabled = value),
              title: const Text('Apply nearest rupee round-off'),
            ),
            _buildTextField('totals_amount_in_words', 'Amount in words', maxLines: 2),
            const SizedBox(height: 24),
            Text(_sectionTitle('Payment'), style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            Wrap(spacing: 16, children: [
              Row(mainAxisSize: MainAxisSize.min, children: [
                Radio<String>(value: 'UPI', groupValue: _paymentMode, onChanged: (v) => setState(() => _paymentMode = v!)),
                const Text('UPI'),
              ]),
              Row(mainAxisSize: MainAxisSize.min, children: [
                Radio<String>(value: 'Bank transfer', groupValue: _paymentMode, onChanged: (v) => setState(() => _paymentMode = v!)),
                const Text('Bank transfer'),
              ]),
              Row(mainAxisSize: MainAxisSize.min, children: [
                Radio<String>(value: 'Other', groupValue: _paymentMode, onChanged: (v) => setState(() => _paymentMode = v!)),
                const Text('Other'),
              ]),
            ]),
            _buildTextField('payment_upi', 'UPI ID'),
            _buildTextField('payment_instructions', 'Payment instructions', maxLines: 2),
            TextFormField(
              controller: _controllers['payment_deeplink'],
              readOnly: true,
              decoration: const InputDecoration(labelText: 'UPI deeplink'),
            ),
            Row(children: [
              ElevatedButton(onPressed: _generatePayLink, child: const Text('Generate Pay Link')),
              const SizedBox(width: 12),
              OutlinedButton(onPressed: _copyDeeplink, child: const Text('Copy Link')),
            ]),
            const SizedBox(height: 24),
            Text(_sectionTitle('Terms'), style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            _buildTextField('terms_title', 'Terms title'),
            _buildTextField('terms_bullets', 'Terms bullets (one per line)', maxLines: 4),
            const SizedBox(height: 24),
            Text(_sectionTitle('Notes'), style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            _buildTextField('notes', 'Notes', maxLines: 3),
            const SizedBox(height: 24),
            Text(_sectionTitle('Branding'), style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            _buildTextField('branding_logo', 'Logo path'),
            _buildTextField('branding_accent', 'Accent color (hex)'),
            _buildTextField('branding_footer', 'Footer text', maxLines: 2),
            const SizedBox(height: 24),
            if (_docType == 'QUOTATION') ...[
              Text(_sectionTitle('Quotation options'), style: Theme.of(context).textTheme.titleMedium),
              const SizedBox(height: 8),
              _buildTextField('quotation_title', 'Quote title'),
              _buildTextField('quotation_validity_days', 'Validity days', inputType: TextInputType.number),
              _buildTextField('quotation_offer_type', 'Offer type'),
              SwitchListTile(
                value: _pricingOptionsEnabled,
                onChanged: (value) => setState(() => _pricingOptionsEnabled = value),
                title: const Text('Enable pricing options (Option A/B/C)'),
              ),
              _buildTextField('quotation_advance_percent', 'Advance percent', inputType: TextInputType.number),
              _buildTextField('quotation_delivery_window', 'Delivery window'),
              _buildTextField('quotation_inclusions', 'Inclusions (one per line)', maxLines: 3),
              _buildTextField('quotation_exclusions', 'Exclusions (one per line)', maxLines: 3),
              _buildTextField('quotation_assumptions', 'Assumptions (one per line)', maxLines: 3),
              _buildTextField('quotation_warranty', 'Warranty / Support'),
              _buildTextField('quotation_change_rate', 'Change requests rate'),
              const SizedBox(height: 24),
            ],
            if (_docType == 'TAX INVOICE') ...[
              Text(_sectionTitle('Invoice specifics'), style: Theme.of(context).textTheme.titleMedium),
              const SizedBox(height: 8),
              _buildTextField('invoice_supply_date', 'Supply date', readOnly: true,
                  onTap: () => _pickDate(_controllers['invoice_supply_date']!)),
              SwitchListTile(
                value: _billToShipSame,
                onChanged: (value) => setState(() => _billToShipSame = value),
                title: const Text('Bill-to and ship-to are the same'),
              ),
              if (!_billToShipSame)
                _buildTextField('invoice_ship_to', 'Ship-to address', maxLines: 2),
              _buildTextField('invoice_place_of_supply', 'Place of supply', requiredField: true),
              SwitchListTile(
              ...
              Row(children: [
                Expanded(child: _buildTextField('invoice_ack_no', 'Ack no.')),
                const SizedBox(width: 8),
                Expanded(
                  child: _buildTextField('invoice_ack_date', 'Ack date', readOnly: true,
                      onTap: () => _pickDate(_controllers['invoice_ack_date']!)),
                ),
              ]),
              Row(children: [
                Expanded(child: _buildTextField('invoice_tcs_percent', 'TCS %', inputType: TextInputType.number)),
                const SizedBox(width: 8),
                Expanded(child: _buildTextField('invoice_tds_note', 'TDS note', maxLines: 2)),
              ]),
              _buildTextField('invoice_transport', 'Transport details'),
              SwitchListTile(
                value: _roundOffEnabled,
                onChanged: (value) => setState(() => _roundOffEnabled = value),
                title: const Text('Round off on grand total'),
              ),
              const SizedBox(height: 24),
            ],
            if (_docType == 'PROJECT BRIEF') ...[
              Text(_sectionTitle('Project brief header'), style: Theme.of(context).textTheme.titleMedium),
              const SizedBox(height: 8),
              _buildTextField('brief_title', 'Project title', requiredField: true),
              _buildTextField('brief_objective', 'Objective', maxLines: 3, requiredField: true),
              _buildTextField('brief_success_metrics', 'Success metrics (one per line)', maxLines: 3),
              _buildTextField('brief_scope', 'In scope (one per line)', maxLines: 3, requiredField: true),
              _buildTextField('brief_out_scope', 'Out of scope (one per line)', maxLines: 3),
              _buildTextField('brief_deliverables', 'Deliverables (one per line)', maxLines: 3, requiredField: true),
              _buildTextField('brief_assumptions', 'Assumptions (one per line)', maxLines: 3),
              const SizedBox(height: 16),
              Text(_sectionTitle('Milestones'), style: Theme.of(context).textTheme.titleMedium),
              ..._buildMilestoneRows(),
              Align(
                alignment: Alignment.centerLeft,
                child: ElevatedButton.icon(
              ...
              const SizedBox(height: 16),
              Text(_sectionTitle('Billing plan'), style: Theme.of(context).textTheme.titleMedium),
              ..._buildBillingRows(),
              Align(
                alignment: Alignment.centerLeft,
                child: ElevatedButton.icon(
                  ...
                  icon: const Icon(Icons.add),
                  label: const Text('Add billing part'),
                ),
              ),
              const SizedBox(height: 8),
              _buildTextField('brief_timeline_days', 'Timeline days', inputType: TextInputType.number, requiredField: true),
              _buildTextField('brief_risks', 'Risks (one per line)', maxLines: 3),
              _buildTextField('brief_communication', 'Communication cadence'),
              _buildTextField('brief_change_control', 'Change control'),
              _buildTextField('brief_acceptance', 'Acceptance criteria (one per line)', maxLines: 3),
            ...
            ExpansionTile(
              ...
              children: [
                SwitchListTile(
                  ...
                  onChanged: (value) => setState(() => _discountAmountOnly = value),
                  title: const Text('Discount mode: per-line amount only'),
                ),
                ListTile(
                  title: const Text('Branding options'),
                  subtitle: const Text('Logo, accent color, footer text available above.'),
                ),
              ],
            ),
